...
@Component({
  selector: 'app-view-by-id-product-category',
  templateUrl: './view-by-id-product-category.component.html',
  styleUrls: ['./view-by-id-product-category.component.css']
})
export class ViewByIdProductCategoryComponent implements OnInit, OnDestroy {
  ...
  data(paramsId){
    const sendDataById = {
      ...
      read: {
        ...
        response: "response-read-by-id-product-category"
      }
    }
  ...
  read() {
    this.connectServ.response$.pipe(takeUntil(this.subs)).subscribe(res => {
      this.dataProductCategory.name = res.data[0].productCategoryName;
    })
  }
  ...
  dataUpdate(paramsId, profileId) {
    let sendDataUpdate = {
      numberOfTable: 1,
      response: "response-update-product-category",
      action: {
        table: "update",
        upload: false,
        join: false,
      ...
      create:[
        {
          ...
          data: {
            ...
            description: "Updated data product category"
          },
          condition: {
            read: false,
            insertId: true,
      ...
      update:[
        {
          ...
          data: "productCategoryName='"+this.dataProductCategory.name+"'",
          condition: {
            read: false
          },
          response: "response-update-product-category",
          toast: {
            name:  "response-update-product-category",
            type: 'update',
            messageToastSuccess: 'Update data product category successfully',
            messageToastError: 'Update data product category not successfully'
          }
        }  
      ],
      createJoinId: [
      ...
      read: {
        ...
        response: "response-read-product-category"
      },
      uploadOffline: {
    ...
    if(navigator.onLine) {
      ...
      this.connectServ.saveOfflineData('update_product_category', sendDataUpdate);
    }
  }
