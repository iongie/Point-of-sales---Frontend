...
@Component({
  selector: 'app-view-kitchen-type',
  templateUrl: './view-kitchen-type.component.html',
  styleUrls: ['./view-kitchen-type.component.css']
})
export class ViewKitchenTypeComponent implements OnInit, OnDestroy {
  header = ["#", "Name"];
  url = ['/kitchen-type/add', 'searchData()'];
  searchEvent = false;
  ...
  read() {
    this.connectServ.response$.pipe(takeUntil(this.subs)).subscribe(res => {
      const changeData = res.data.map(x => {
        const data = {
          idOne: x.kitchenTypeId,
          tableOne: x.kitchenTypeName
        }
        return data;
      });
  ...
  offlineData(changeData) {
    ...
    JSON.stringify('view_kitchen_type'), this.key, {
      keySize: 16,
      iv: this.iv,
  ...
  dataUpdate(ev) {
    ...
    this.router.navigate(['kitchen-type/detail/'+ convertEncryptedId]);
  }
  ...
  actionDelete(ev, data, profileId){
    let sendDataDelete = {
      numberOfTable: 1,
      response: "response-delete-kitchen-type",
      action: {
        table: "delete",
        upload: false,
        join: false,
      ...
      create:[  
        {
          ...
          data: {
            ...
            description: "Deleted data kitchen type"
          },
          condition: {
            read: false,
            insertId: true,
      ...
      delete:[
        {
          ...
          response: "response-delete-kitchen-type",
          toast: {
            name:  "response-delete-kitchen-type",
            type: 'delete',
            messageToastSuccess: 'delete data kitchen type successfully',
            messageToastError: 'delete data kitchen type not successfully'
          }
        }  
      ],
      createJoinId: [
    ...
    if(navigator.onLine) {
      ...
      this.connectServ.saveOfflineData('delete_kitchen_type', sendDataDelete);
    }
  }
}
